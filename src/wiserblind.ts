...
import { GroupSetEvent } from './models.js';
...
export class WiserBlind extends WiserAccessory {
  ...
  async setTarget(value: CharacteristicValue) {
    ...
    this.platform.log.debug(
      `Homekit set ${this.name} to ${this.targetPosition} (${this.toWiserLevel(
        this.targetPosition,
      )})`,
    );
    this.wiser.setGroupLevel(
      this.device.wiserProjectGroup.address,
      this.toWiserLevel(this.targetPosition),
    );
  }
  ...
  async getCurrent(): Promise<CharacteristicValue> {
    ...
    this.platform.log.debug(
      `Get current position ${this.name}(${this.id}) ${percent}`,
    );
    return percent;
  }
  ...
  async getPositionState(): Promise<CharacteristicValue> {
    ...
    this.platform.log.debug(
      `Get position state ${this.name}(${this.id}) ${positionState}`,
    );
